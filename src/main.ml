  ...
  lazy
    (let cmd = Sys.argv.(0) in
     if String.contains cmd '/' then
       (* Do not use Sys.executable_name, which may resolve symlinks: we want
          the original path. (e.g. _build/install/default/bin/foo is a
          symlink) *)
...
let lookup_opam_optimist () =
  (* Querying opam is slow, let's hypothesize that the binary is in opam's bin
     dir. e.g. <OPAM>/<SWITCH>/bin *)
  try
...
let handle_errors file stderr_redir_fd =
  ...
  | [parsing_err] -> (
    let all_groups =
      try
        Re.exec parsing_error_re parsing_err
        |> Re.Group.all
        |> Array.to_list
        |> function _h :: l -> List.map int_of_string l | _ -> []
      with _ -> []
    in
    ...
    | [bl; bc; el; ec] ->
      Format.eprintf
        ...
        bl bc el ec pp_file bl bc
    | _ -> print_internal_error ())
  | [_idempotence; err_loc] ->
    (let all_groups =
       try
         Re.exec parsing_error_re err_loc
         |> Re.Group.all
         |> Array.to_list
         |> function _h :: l -> List.map int_of_string l | _ -> []
       with _ -> []
...
let format_cmd =
  ...
  let f lang in_place skip_idempotence buffer_name file =
    ...
    let language =
      ...
      | None, `File f -> (
        let ext =
          ...
          if fext = ".md" then Filename.extension (Filename.remove_extension f)
          else fext
        in
        try
          let ext = String.sub ext 1 (String.length ext - 1) in
          List.find_opt (( = ) ext) supported_languages
          |> function
    ...
    let has_cr =
      (* Only start writing to the pipe when the process is ready to read as
         outputting to the pipe might hang if the output is not consumed. *)
